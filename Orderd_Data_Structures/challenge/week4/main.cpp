...
void downHeap(Node *n) {
  ...
  if (n->right && n->right->value < minChild->value) {
    minChild = n->right;
  }

  if (minChild != n) {
    std::swap(n->value, minChild->value);
    downHeap(minChild);
  }
}
...
void printTreeVertical(const Node *n, int depth = 0) {
  ...
  printSpaces(spacesPerLevel * depth);
  std::cout << "└── ";
  printTreeVertical(n->right, depth + 1);

  printSpaces(spacesPerLevel * depth);
  std::cout << "└── ";
  printTreeVertical(n->left, depth + 1);
}
